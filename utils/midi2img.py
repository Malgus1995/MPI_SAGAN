...
def midi2image(midi_path,save_path,count,argv2=1):
    res = []
        ...
        while repetitions < argv2:
            ...
            for i in range(prev_index,len(pitchs)):
                ...
                if dur+start - index*maxSongLength < maxSongLength:
                    ...
                    prev_index = i
                    break
            save_path_ = (os.path.join(save_path,str(count)+'.png'))
            print(midi_path.split("/")[-1].replace(".midi",f"_{instrument_name}_{index}.png").replace('data','img'))
            imwrite(save_path_,matrix)
            index += 1
            repetitions+=1
